...
// 【斷言翻轉｜D50】R11 定稿：`DELETE /api/v1/marks` 下線回 410，清空水位線與
// 拒收一併廢除；原 R3-4 的硬刪、401 守門、clearedAt 輔助 API、水位線拒收與
// changes.clearedAt 帶回水位線等斷言全數作廢。刪雲端改走 CLOUD_DATA_CONTRACT
// （契約測試在 test/mock-sync-server.test.js 的 D50/R11 段）。
const { CLOUD_DATA_CONTRACT } = require('./helpers/mock-sync-server.js');

test('R3-4 DELETE /api/v1/marks：回 410 gone 並指向替代端點，警示與墓碑一筆不動（D50）', async () => {
  const h = harness();
  ...
  assert.equal(h.server.marks.count(), 2);
  assert.equal(h.server.marks.tombstoneCount(), 1);
  assert.equal(h.server.marks.clearedAt(), null, '不再寫清空水位線');
});

test('R3-4 marks sync：updatedAt 早於任何時間點的 upsert 都照收（R11 廢除水位線拒收，D50）', async () => {
  const h = harness();
  const { res, body } = await h.syncJson({
    upserts: [markOf({ key: KEY_A, updatedAt: 1 }), markOf({ key: KEY_B, addedAt: 1, updatedAt: 2 })],
    deletes: [],
  });
  assert.equal(res.status, 200);
  assert.deepEqual(body.applied.upserts.sort(), [KEY_A, KEY_B]);
  assert.deepEqual(body.applied.rejectedIds, []);
});

test('R3-4 marks sync：changes 物件保留 clearedAt 鍵、恆為 null（D50）', async () => {
  const h = harness();
  await h.push([markOf()]);
  const body = (await h.syncJson({ upserts: [], deletes: [], since: 0 })).body;
  assert.notEqual(body.changes, null, '前置條件:帶 since 就有 changes');
  assert.deepEqual(Object.keys(body.changes).sort(), ['clearedAt', 'deleted', 'hasMore', 'marks'], 'changes 固定四欄');
  assert.equal(body.changes.clearedAt, null);
});

test('R3-4 marks sync：沒帶 since／cursor 時 changes 整個為 null', async () => {
  const h = harness();
  const body = (await h.syncJson({ upserts: [], deletes: [] })).body;
  assert.equal(body.changes, null, '首輪回填走 GET，這一次 POST 只領游標');
});

test('R3-4 DELETE /api/v1/links：回 410，不碰 marks（D50）', async () => {
  const h = harness();
