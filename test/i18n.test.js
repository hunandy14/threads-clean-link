...
const SCAM_GUARD_KEYS = [
  ...
  'scamTagLabel',
  'scamTagTooltip',
  // 某作者第一次被判定命中、自動加入本機標記名單時的提示。
  'scamFirstHitToast',
  // 作者已在標記名單中(非本次命中)時，標籤改顯示的理由。
  'scamBlockedByList',
...
test('標記名單卡:十顆選項頁文案 key 在 zh 與 en 兩份字典皆存在且非空', () => {
  for (const locale of ['zh', 'en']) {
...
// 比較划算。
test('標記名單卡:帶插值的文案在 zh 與 en 都保留佔位符({n}/{d}/{name})', () => {
  const PLACEHOLDERS = {
    opScamListCount: '{n}',
    opScamAddedOn: '{d}',
