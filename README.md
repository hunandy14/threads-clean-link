...
**LINE 群組引導警示**:點進 Threads 貼文詳情頁時，就地辨識「長篇心得鋪陳＋末篇引導加 LINE」的招攬串文(判準為 LINE 提及＋群組／加入詞的雙訊號，另保留 LINE 加好友深連結單獨成立、錨點＋投資話術詞，以及同一組 LINE ID 出現在另一位已列名作者身上的跨帳號比對，共四條路)，命中即在作者列時間右邊掛一枚標記(取不到時退回互動列上方)，並把該作者記進警示名單;河道上再遇到同一位作者也會標記。警示名單可在「紀錄與設定」頁管理(附證據連結、可解除、解除後不再自動標記)，**預設只存在這台裝置;登入雲端同步後會同步到你的帳號，不含貼文文字**，任何情況下都不與其他使用者共享，可用總開關(預設開啟)整個停用。詳見 [docs/scam-guard.md](docs/scam-guard.md)。
...
**雲端同步(選用，需 Google 登入)**:預設關閉，於「紀錄與設定」頁主動點擊「使用 Google 帳號登入」後才會啟用。登入後，清理紀錄(清理前後的網址、被移除的追蹤參數、貼文作者名稱與帳號、貼文摘要、清理時間)會同步到開發者自營伺服器，並與手機版「Meta Link Clearer」App 共用同一份紀錄;不同步瀏覽紀錄、cookie 或 Threads 帳號密碼。LINE 群組引導警示的名單也會一併同步(可用該功能的總開關停用)，同步的欄位只有:作者數字 id、帳號與顯示名快照、證據貼文的網址與時間、判定訊號類別，**不含貼文文字**。免費方案雲端保留最新 1,000 筆，本機紀錄不受影響、仍完整保留。登入後另會同步一組隨機產生的裝置識別碼與一個可自訂的裝置名稱，用於標示每筆紀錄來自哪台裝置;裝置清單可在同一頁改名或移除。可隨時在同一頁登出、按「刪除雲端資料並登出」(登出所有裝置、本機資料保留、重新登入後重新上傳，見 `docs/cloud-sync.md` D50)，或刪除帳號(伺服器端立即永久刪除)。未登入時，本擴充功能不會連線到任何自建伺服器。詳見下方[運作原理與隱私](#運作原理與隱私)。
...
- LINE 群組引導警示的名單本體、證據片段與已解除條目只寫在 `chrome.storage.local`，備援請求的節流表(記「哪個網址剛問過」與「近一分鐘問了幾次」)只寫在 `chrome.storage.session`、瀏覽器關閉即清。名單**預設只存在這台裝置**;登入下方的「雲端同步」之後會同步到你的帳號，同步範圍只有作者帳號與顯示名、證據貼文的網址與時間，**不含貼文文字片段**，也不含你當時開的那一頁網址——登出即停止，總開關關閉期間不拉也不推。節流表一律不上傳;名單不進匯出／匯入檔，也不做社群回報或跨使用者共享。偵測全在本機完成，只在使用者自己點進貼文詳情頁時掃描該頁;僅當頁面本身沒帶作者識別碼時，才對同一篇貼文的永久連結補發一次匿名請求(不帶登入憑證、不帶 cookie)，總開關關閉時一律不發。
...
- 已登入時，設定頁會從 Google 載入你的帳號大頭照用於顯示;其網址與下方帳號身分資料一併存於後端，用於識別使用者，不另外保存圖片本身。
- 後端是開發者自營的伺服器，也是手機版 App「Meta Link Clearer」共用的同一套帳號與資料，讓兩邊的清理紀錄互相同步;為了識別使用者身分，後端會另外保存一份帳號基本資料(Google 帳號 email、名稱、大頭照網址)，僅用於登入識別與介面顯示。
- 同步的清理紀錄資料涵蓋:清理前後的網址、被移除的追蹤參數、貼文作者名稱與帳號、貼文摘要、清理時間。**不會**同步瀏覽紀錄、cookie、Threads 帳號密碼，或任何未經清理的頁面內容。
- 登入後，LINE 群組引導警示的名單也會一併同步(可用該功能的總開關停用;關閉期間不拉也不推)。同步的欄位只有:作者數字 id、帳號與顯示名快照、證據貼文的網址、掃到的時間與貼文發布時間、判定訊號類別(link／line／group／join／pitch，不含命中的原文)、規則版本，以及回報這筆證據的裝置識別碼。**貼文文字片段一律不上傳**，只留在本機;你當時開的那一頁網址(`postUrl`)這個欄位本身也不上傳，但缺錨點篇的舊證據會以它的值充當證據貼文網址一併同步。解除與復原同樣跨裝置一致。
- 登入後另會同步一組隨機產生的裝置識別碼與一個可自訂的裝置名稱，用途只有一個:標示每筆紀錄來自哪台裝置。識別碼隨機產生，與你的 Google 帳號、裝置硬體皆無關，本身不含硬體序號、IP 或地理位置;預設的裝置名稱只由瀏覽器名稱與瀏覽器回報的作業系統類別組成(例如「Chrome on Windows」)，可隨時在「紀錄與設定」頁改名。裝置也可以從清單移除;移除後，既有紀錄仍會顯示原本的裝置名稱。
- 免費方案雲端保留最新 1,000 筆;本機紀錄不受影響，一律完整保留(不隨雲端上限被裁切)。
- 可隨時在「紀錄與設定」頁登出、清除雲端紀錄，或直接刪除帳號——登出或刪除帳號時，前述帳號身分資料(email、名稱、大頭照網址)會一併從伺服器移除;清除雲端紀錄與刪除帳號都會**連同警示名單**一併從伺服器永久刪除(硬刪＋墓碑)，不可復原，本機那一份名單不受影響、仍完整保留。
