...
- 登入後另會同步一組隨機產生的裝置識別碼(與你的 Google 帳號、裝置硬體皆無關)與一個可自訂的裝置名稱(預設是「Chrome on Windows」這類字樣，只由瀏覽器回報的作業系統類別組成)，用途只有一個:標示每筆紀錄來自哪台裝置。不會取得裝置型號、序號、IP、地理位置或任何其他裝置指紋;可隨時在「紀錄與設定」頁改名，或把裝置從清單移除。
