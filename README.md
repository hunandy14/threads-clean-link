...
**LINE 群組引導警示**:點進 Threads 貼文詳情頁時，就地辨識「長篇心得鋪陳＋末篇引導加 LINE」的招攬串文(判準為 LINE 提及＋群組／加入詞的雙訊號，另保留 LINE 加好友深連結單獨成立與錨點＋投資話術詞兩條路)，命中即在作者列時間右邊掛一枚標記(取不到時退回互動列上方)，並把該作者記進本機警示名單;河道上再遇到同一位作者也會標記。警示名單可在「紀錄與設定」頁管理(附證據連結、可解除、解除後不再自動標記)，**只存在這台裝置、不上傳、不與其他使用者共享**，可用總開關(預設開啟)整個停用。詳見 [docs/scam-guard.md](docs/scam-guard.md)。
...
- LINE 群組引導警示的名單本體、證據片段與已解除名單只寫在 `chrome.storage.local`，備援請求的節流表(記「哪個網址剛問過」與「近一分鐘問了幾次」)只寫在 `chrome.storage.session`、瀏覽器關閉即清;兩者都**只存在這台裝置**:不上傳、不隨雲端同步、不進匯出檔，也不做社群回報或跨使用者共享。偵測全在本機完成，只在使用者自己點進貼文詳情頁時掃描該頁;僅當頁面本身沒帶作者識別碼時，才對同一篇貼文的永久連結補發一次匿名請求(不帶登入憑證、不帶 cookie)，總開關關閉時一律不發。
