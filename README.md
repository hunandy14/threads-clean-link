...
`/share/XXXX` 短碼是一次性代碼,對應關係僅存於 Meta 伺服器,不含可離線解析的資訊。還原短碼的唯一方式,是對該連結發出一次請求,讀取轉址後的最終網址。
...
短碼解析請求皆不攜帶登入憑證,不會以使用者身分送出;但仍會讓 Threads 伺服器看到請求來源 IP 與瀏覽器特徵,此為任何網路請求的固有限制。需要避免 IP 層級關聯者,可搭配 VPN,或僅使用右鍵功能而不依賴自動淨化。
