...
**LINE 群組引導警示**:點進 Threads 貼文詳情頁時，就地辨識「長篇心得鋪陳＋末篇引導加 LINE」的招攬串文(判準為 LINE 提及＋群組／加入詞的雙訊號，另保留 LINE 加好友深連結單獨成立與錨點＋投資話術詞兩條路)，命中即在作者列時間右邊掛一枚標記(取不到時退回互動列上方)，並把該作者記進警示名單;河道上再遇到同一位作者也會標記。警示名單可在「紀錄與設定」頁管理(附證據連結、可解除、解除後不再自動標記)，**預設只存在這台裝置;登入雲端同步後會同步到你的帳號，不含貼文文字**，任何情況下都不與其他使用者共享，可用總開關(預設開啟)整個停用。詳見 [docs/scam-guard.md](docs/scam-guard.md)。
