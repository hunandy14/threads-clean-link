...
**雲端同步(選用，需 Google 登入)**:預設關閉，於「紀錄與設定」頁主動點擊「使用 Google 帳號登入」後才會啟用。登入後，清理紀錄(乾淨網址、被移除的追蹤參數、貼文作者名稱與帳號、貼文摘要、清理時間)會同步到開發者自營伺服器，並與手機版「Meta Link Clearer」App 共用同一份紀錄;不同步瀏覽紀錄、cookie 或 Threads 帳號密碼。免費方案雲端保留最新 1,000 筆，本機紀錄不受影響、仍完整保留。可隨時在同一頁登出、清除雲端紀錄，或刪除帳號(伺服器端立即永久刪除)。未登入時，本擴充功能不會連線到任何自建伺服器。詳見下方[運作原理與隱私](#運作原理與隱私)。
