...
- 登入後，LINE 群組引導警示的名單也會一併同步(可用該功能的總開關停用;關閉期間不拉也不推)。同步的欄位只有:作者數字 id、帳號與顯示名快照、證據貼文的網址、掃到的時間與貼文發布時間、規則版本，以及回報這筆證據的裝置識別碼。**貼文文字片段與你當時開的那一頁網址不會上傳**，只留在本機;解除與復原同樣跨裝置一致。
...
- 可隨時在「紀錄與設定」頁登出、清除雲端紀錄，或直接刪除帳號——登出或刪除帳號時，前述帳號身分資料(email、名稱、大頭照網址)會一併從伺服器移除;刪除帳號另會將雲端清理紀錄一併永久刪除，不可復原。
