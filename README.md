...
**投資詐騙串文警示**:點進 Threads 貼文詳情頁時，就地辨識「長篇投資心得＋末篇引導加 LINE」的招攬串文(判準為加 LINE 錨點＋投資話術詞的雙條件)，命中即在互動列上方掛一枚警示標記，並把該作者記進本機黑名單;河道上再遇到同一位作者也會標記。黑名單可在「紀錄與設定」頁管理(附證據連結、可解除、解除後不再自動加回)，**只存在這台裝置、不上傳、不與其他使用者共享**，可用總開關(預設開啟)整個停用。詳見 [docs/scam-guard.md](docs/scam-guard.md)。
