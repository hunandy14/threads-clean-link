...
- LINE 群組引導黑名單的黑名單、證據片段與已解除名單只寫在 `chrome.storage.local`，備援請求的節流表(記「哪個網址剛問過」與「近一分鐘問了幾次」)只寫在 `chrome.storage.session`、瀏覽器關閉即清;兩者都**只存在這台裝置**:不上傳、不隨雲端同步、不進匯出檔，也不做社群回報或跨使用者共享。偵測全在本機完成，只在使用者自己點進貼文詳情頁時掃描該頁;僅當頁面本身沒帶作者識別碼時，才對同一篇貼文的永久連結補發一次匿名請求(不帶登入憑證、不帶 cookie)，總開關關閉時一律不發。
