...
- 通:沿用既有，不做任何事——後端可能是使用者自己另開一個終端機跑的，也可能是本工具上一次啟動、至今還留著的，兩者對工具而言無法區分也不必區分。
- 不通:在後端目錄背景跑起 `npm run dev`(內部即 `wrangler dev`)，輸出導向固定的 log 檔(每次啟動覆寫，不是續寫，讀最新一次的內容不必先清空)，PID 寫進固定的 PID 檔，接著輪詢 `/health` 最多 60 秒;通過才繼續往下走，逾時就印出 log 檔尾段、把剛啟動的行程樹連根殺掉、非 0 退出。**不碰瀏覽器**——後端沒起來的話後面每一步都是白做，還會留下一個連錯環境的瀏覽器要收拾。
...
自動啟動的後端是**常駐**的:工具本身結束不會連帶關掉它，這樣才能讓後端跨多次執行留著、不必每次都重新等編譯。需要關閉時用同一支工具的獨立旗標(不需要 `--env`，因為關後端跟連線目標無關)：讀 PID 檔、殺掉整棵行程樹(只殺父行程留下子行程繼續佔著埠是最常見的坑)、刪 PID 檔、輪詢埠釋放後才回報成功。PID 檔不存在但埠仍有人聽，代表後端不是這支工具啟動的，印提示要求使用者自行處理，不擅自動手。
