...
**(a) 前置探活，探不到就自動帶起來。** local 依賴開發機自己跑著的後端，這件事工具無法代勞，但可以在動任何東西之前先確認，通不通則決定接下來自己動手還是求救。`--env local` 啟動前先 `GET http://localhost:8787/health`:

- 通:沿用既有，不做任何事，做完後面 Chrome 的步驟就正常結束——後端可能是使用者自己另開一個終端機跑的，也可能是本工具上一次以 `--detach-backend` 啟動、至今還留著的，兩者對工具而言無法區分也不必區分。
- 不通:預設在後端目錄以**前景**跑起 `npm run dev`(內部即 `wrangler dev`)，比照一般網站的 dev server——輸出直接印在目前這個終端機(不開 log 檔，`wrangler` 的顏色與互動提示原樣可用)，PID 仍寫進固定的 PID 檔(供意外情況下用 `--stop-backend` 救孤兒)，接著輪詢 `/health` 最多 60 秒;通過才繼續往下走做 Chrome 的步驟，做完之後工具**卡在前景**繼續轉印後端輸出——按 Ctrl+C(SIGINT/SIGTERM)會把後端整棵一併關掉再結束；後端自己先掛掉(不是使用者按 Ctrl+C)則印結束碼並非 0 結束。逾時就提示往上檢查終端機輸出、把剛啟動的行程樹連根殺掉、非 0 退出。**不碰瀏覽器**——後端沒起來的話後面每一步都是白做，還會留下一個連錯環境的瀏覽器要收拾。
- `--detach-backend`:改回背景常駐——不佔用終端機，輸出導向固定的 log 檔(每次啟動覆寫，不是續寫，讀最新一次的內容不必先清空)，工具本身結束不會連帶關掉後端，之後重跑會直接沿用既有。想讓終端機空出來做別的事、或需要背景長駐時用這個。
- `--no-backend`:退回最早的行為，探不到就只印手動啟動提示並非 0 退出，不自動動手。CI 或不想讓工具背著自己啟動長駐行程的場景用這個。
...
關閉後端用同一支工具的獨立旗標 `--stop-backend`(不需要 `--env`，因為關後端跟連線目標無關)：讀 PID 檔、殺掉整棵行程樹(只殺父行程留下子行程繼續佔著埠是最常見的坑)、刪 PID 檔、輪詢埠釋放後才回報成功。前景模式正常按 Ctrl+C 結束不需要它——那條路徑自己就會做完同一套收尾;`--stop-backend` 是給 `--detach-backend` 的背景後端，或前景模式的父行程被強制關閉(例如 `kill -9`、終端機視窗被直接叉掉)後留下的孤兒行程用的救援手段。PID 檔不存在但埠仍有人聽，代表後端不是這支工具啟動的，印提示要求使用者自行處理，不擅自動手。
...
| 跨 repo 後端路徑 | `TCL_API_LOCAL_DIR` 環境變數覆寫，預設猜 `~/.threads-clean-link/api-local/api` | 同一原則:用環境變數指路徑，不要把另一個 repo 的絕對路徑寫死進程式碼 |

檢查表本身的重點：先確認目標專案的 build 工具是否已經有自己的一套環境字（如 EAS 的 `--profile`），本文的 `--env` 只管「連線目標」這個維度，不要跟該工具既有、語意不同的維度旗標打架——寧可在文件裡寫清楚兩者對照，也不要為了統一字面而覆蓋掉工具原生慣例。跨 repo 後端用環境變數指路徑，不硬綁：後端十之八九是另一個 repo 的 worktree 或 clone，路徑因人而異，把它寫死在程式碼裡只會讓下一個開發機或下一個人踩坑；用環境變數帶預設猜測值，讓使用者需要時能覆寫，不需要時直接吃預設。
