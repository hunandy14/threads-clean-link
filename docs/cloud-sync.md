...
| — | **D30–D34 為「投資詐騙串文警示」的決策。該功能純屬本機、與雲端同步無關，列於此表只是為了讓決策編號在單一序列上連續。** | 決策編號分散在多份文件會讓「下一號是幾號」無從判斷，重號的代價高於主題混列 | 2026-09-19 |
| D30 | 投資詐騙黑名單（含證據與白名單）只寫 `chrome.storage.local`，備援請求的節流表只寫 `chrome.storage.session`（瀏覽器關閉即清），**皆不上雲**：不進同步佇列、不進 `syncState`、不進匯出／匯入檔，也不做任何跨使用者的共享名單 | 名單是對真人帳號的負面標記，誤判成本高且無從申訴；一旦上雲就等於由開發者代為散布指控，還會牽出審核責任與濫用風險。留在本機則始終是「這台裝置上的使用者自己的判斷」，隨時可整包清掉 | 2026-09-19 |
| D31 | 作者數字 id 主路徑取自貼文詳情頁的伺服器預載資料（零請求）；僅在該處缺席時，才對同一篇貼文的永久連結發一次匿名請求（不帶登入憑證）作備援，且回應須帶回相符的帳號名稱才採信，同一篇貼文 24 小時只發一次、另有每分鐘全域上限，總開關關閉時一律不發。取不到 id 時頁面照樣掛警示標記，但不寫入名單 | 頁面本來就已經把作者 id 送到瀏覽器，再去問一次是多餘的網路行為。備援不可省是因為缺了 id 就只剩帳號可認，改名即失聯；但備援必須受限——交叉驗證擋的是張冠李戴，節流與開關擋的是被誤用成爬蟲，「沒有 id 就不入名單」擋的是主鍵語意被破壞 | 2026-09-19 |
...
| D33 | 命中門檻採「錨點＋至少一個強話術詞」的雙條件（連結型錨點可單獨成立），弱詞只列入證據不計門檻，並對「賴」前的信／依／無／仰／倚設負向邊界 | 誤殺成本高於漏抓：被誤標的是一個真人帳號，標錯會讓使用者不再信任整個功能；漏抓只是回到沒有這個功能的狀態。單一條件兩邊都站不住——交友貼文會留 LINE、討論區會聊飆股；而「信賴：」這類句子常同時帶投資詞，話術詞的二次確認擋不住，必須從錨點本身排除 | 2026-09-19 |
