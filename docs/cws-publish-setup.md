...
這份文件只需要做**一次**。做完之後,`.github/workflows/release.yml` 的「上傳新版草稿至 Chrome Web Store」步驟就會在每次發版時,自動把打包好的 zip 上傳成商店的新版本草稿——**但不會幫你按送審**,送審這一步刻意保留給人工在主控台親自確認再按下去。
...
Google Cloud 建專案
  ...
  → 之後每次 push 到 main 且版本號有變動,CI 就會自動把新版 zip 上傳成草稿,
    你只要去主控台按「提交審查」
```
...
- **CI(`release.yml`)自動做的事**:用 refresh token 換一次性的 access token,把該版本的 zip 用 `PUT` 上傳到 Chrome Web Store,讓它成為這個項目的**新版本草稿**。
- **CI 刻意不做的事**:呼叫 `publish` API 把草稿送審。這是團隊拍板的決定——每次要不要送審、什麼時機送審,保留給人工在開發者主控台親眼確認草稿內容(版本號、變更內容)無誤後再手動點擊「提交審查」。
- 所以每次看到 CI 印出「草稿已上傳,請至開發者主控台按提交審查」,代表輪到你上主控台按下去了。
