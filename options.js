...
(function (root) {
  ...
  // UI 全面對齊手機任務，work item 7:original/removedParams 兩列渲染——
  // 另一車道正把這兩個欄位存進 schema(original 字串、removedParams
  // [{name,value}])，本分支不含該 schema 變更，故比照 seen[] 已經踩過的
  // 同一個坑:sanitizeEntries 若不主動把它們「路過」保留，即使合併後
  // storage 真的有這兩個欄位，渲染端也永遠讀不到，detailExtraRows 會變成
  // 永遠不出現的死功能。這裡只認形狀(name/value 皆為字串才收)，不強求
  // 語意正確性，語意驗證交給寫入端(另一車道)。
  function sanitizeRemovedParams(list) {
    if (!Array.isArray(list)) return [];
    var out = [];
    for (var i = 0; i < list.length; i++) {
      var p = list[i];
      if (!p || typeof p.name !== 'string' || p.name === '' || typeof p.value !== 'string') continue;
  ...
  function sanitizeEntries(list) {
    ...
    return list
      ...
      .map(function (e) {
        ...
        // original/removedParams:見上方 sanitizeRemovedParams 註解，同一個
        // 「路過保留」道理，只認形狀。
        if (typeof e.original === 'string' && e.original !== '') out.original = e.original;
        var removedParams = sanitizeRemovedParams(e.removedParams);
        if (removedParams.length > 0) out.removedParams = removedParams;
        return out;
      });
  }
  ...
  // 手機版 CopyRow 的「原始連結」「追蹤參數 {name}」兩類列。另一車道正把
  // original/removedParams 存進 schema，本分支不含該變更，故防禦寫法:
  // entry.original 缺席/非字串/與 cleaned 相同(手機版:
  // item.original !== item.cleaned 才顯示)一律不產生該列；
  // entry.removedParams 缺席/非陣列/陣列內項目形狀不對(缺 name/value 或
  ...
  function buildDetailExtraRows(entry) {
    ...
    params.forEach(function (p) {
      if (!p || typeof p.name !== 'string' || p.name === '' || typeof p.value !== 'string') return;
      rows.push({ type: 'param', name: p.name, display: p.value, copyValue: p.value });
    });
    return rows;
  }
